...
let rec print_t ppf = function
  ...
  | Var(a,t) -> fprintf ppf "Var(\"%s\",%a)@?" a Type.print_t t
...
let rec visit(env:S.t) (e: Ast.t):t =
  ...
  (* ムータブルな変数は中身を取り出す *)
  | Ast.Var(id) when S.mem id env -> Get(Var(id, Type.gentyp()),Int 0)
  (* ムータブルな変数のリファレンスはそのままのポインタ値　*)
  | Ast.Var(id) -> Var(id, Type.gentyp())
  (*  | Ast.Ref(Ast.Var(id)) when S.mem id env -> Var(id) *)
  | Ast.Array (e1, e2) -> Array(visit env e1, visit env e2)
  | Ast.Ref(Ast.Var(id)) when S.mem id env -> Var(id, Type.gentyp())
